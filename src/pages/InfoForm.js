...
const apiUrl = "https://form-server-ze5tahv2zq-as.a.run.app/";
const serviceURL = "https://docx-converter-ze5tahv2zq-as.a.run.app/airtable_api";
const idQueryServiceURL = "https://docx-converter-ze5tahv2zq-as.a.run.app/airtable_staff";
...
{    
    ...
    async fetchInfo(tmp) {
        ...
        map[PROVINCE_CODE_FIELD] = "district_opts";
        map[DISTRICT_CODE_FIELD] = "ward_opts";        
        map[PROVINCE_CODE_OF_HOSPITAL_FIELD] = "hospital_opts"
        ...
        try {
          ...
          runInAction( () => {
            ...
            // }
            this.record = tmp;
            this.current_addr_district_opts = opts[CURRENT_ADDR_FIELD] ["district_opts"];
            this.current_addr_ward_opts = opts[CURRENT_ADDR_FIELD] ["ward_opts"];
            this.perm_addr_district_opts = opts[PERM_ADDR_FIELD] ["district_opts"];
            this.perm_addr_ward_opts = opts[PERM_ADDR_FIELD] ["ward_opts"];
            this.birth_district_opts = opts[BIRTHPLACE_FIELD] ["district_opts"];
            this.birth_ward_opts = opts[BIRTHPLACE_FIELD] ["ward_opts"];
            this.hospital_opts = opts["hospital_opts"];
            this.hospital_search = opts["hospital_search"];
          } );
...
function fromRecordArray(arr){
  ...
  let idx = 0;
  data["Mã NV"] = arr[idx++];
  data["Họ và tên"] = arr[idx++];
  data["Sinh ngày"] = arr[idx++];
  data["Nơi sinh"] = arr[idx++];
  data["Giới tính"] = arr[idx++];
  data["Hôn nhân"] = arr[idx++];
  data["Số CMND"] = arr[idx++];
  data["Ngày cấp"] = arr[idx++];
  data["Nơi cấp"] = arr[idx++];
  data["Quốc tịch"] = arr[idx++];
  data["Dân tộc"] = arr[idx++];
  data["Tôn giáo"] = arr[idx++];
  data["Trình độ"] = arr[idx++];
  data["Ngành đào tạo"] = arr[idx++];
  data["Chức vụ/chức danh"] = arr[idx++];
  data["Điện thoại"] = arr[idx++];
  data["Email"] = arr[idx++];

  for (let key of [CURRENT_ADDR_FIELD, PERM_ADDR_FIELD,BIRTHPLACE_FIELD]) {
    let obj = {}
    obj["Name"] = arr[idx++];
    obj["Mã tỉnh thành"] = arr[idx++];
    obj["Mã quận huyện"] = arr[idx++];
    obj["Mã phường xã"] = arr[idx++];
    obj["Số nhà, tên phố"] = arr[idx++];
    obj["Tỉnh, thành phố"] = arr[idx++];
    obj["Quận, huyện"] = arr[idx++];
    obj["Phường, xã"] = arr[idx++];
    data[key] = obj;
  }

  data["Mã tỉnh thành nơi KCB"] = arr[idx++];
  data["Mã bệnh viện nơi KCB"] = arr[idx++];
  data["Tỉnh thành nơi KCB"] = arr[idx++];
  data["Nơi khám chữa bệnh"] = arr[idx++];
  return data;
}
...
function toRecordArray(data){
  ...
  let idx = 0;
  arr[idx++] = data["Mã NV"];
  arr[idx++] = data["Họ và tên"];
  arr[idx++] = data["Sinh ngày"];
  arr[idx++] = data["Nơi sinh"];
  arr[idx++] = data["Giới tính"];
  arr[idx++] = data["Hôn nhân"];
  arr[idx++] = data["Số CMND"];
  arr[idx++] = data["Ngày cấp"];
  arr[idx++] = data["Nơi cấp"];
  arr[idx++] = data["Quốc tịch"];
  arr[idx++] = data["Dân tộc"];
  arr[idx++] = data["Tôn giáo"];
  arr[idx++] = data["Trình độ"];
  arr[idx++] = data["Ngành đào tạo"];
  arr[idx++] = data["Chức vụ/chức danh"];
  arr[idx++] = data["Điện thoại"];
  arr[idx++] = data["Email"];

  for (let key of [CURRENT_ADDR_FIELD, PERM_ADDR_FIELD, BIRTHPLACE_FIELD]) {
    let obj = data[key];
    arr[idx++] = (obj && obj["Name"]) ? obj["Name"] : "";
    arr[idx++] = (obj && obj["Mã tỉnh thành"]) ? obj["Mã tỉnh thành"] : "";
    arr[idx++] = (obj && obj["Mã quận huyện"]) ? obj["Mã quận huyện"] : "";
    arr[idx++] = (obj && obj["Mã phường xã"]) ? obj["Mã phường xã"] : "";
    arr[idx++] = (obj && obj["Số nhà, tên phố"]) ? obj["Số nhà, tên phố"] : "";
    arr[idx++] = (obj && obj["Tỉnh, thành phố"]) ? obj["Tỉnh, thành phố"] : "";
    arr[idx++] = (obj &&  obj["Quận, huyện"]) ? obj["Quận, huyện"] : "";
    arr[idx++] = (obj && obj["Phường, xã"]) ? obj["Phường, xã"] : "";    
  }

  arr[idx++] = data["Mã tỉnh thành nơi KCB"];
  arr[idx++] = data["Mã bệnh viện nơi KCB"];
  arr[idx++] = data["Tỉnh thành nơi KCB"];
  arr[idx++] = data["Nơi khám chữa bệnh"];
  return arr;
}
...
const InfoForm = observer( () => {
  ...
  const [record,set_record] = useQueryState("r",[])
  ...
  useAsyncEffect( async () => {
    if (id && id!="") {        
        ...
        if (!record || record.length==0){          
            ...
            let tmp = res.data;
            if (tmp) tmp[PRIMARY_FIELD] = id;
            await fillForm(form, tmp);            
            // let arr = toRecordArray(store.record);
  ...
  const onFinish = async (values) => {
    ...
    if (store && store.record){
      ...
      let out = {};
      for (let key in sample_record){
        let val = values[key];
        let prev = record[key];
        if (key==CURRENT_ADDR_FIELD || key==PERM_ADDR_FIELD || key==BIRTHPLACE_FIELD){
          let ok = false;
          for (let k of [PROVINCE_CODE_FIELD, DISTRICT_CODE_FIELD, WARD_CODE_FIELD]){
            let vk = val[k];
            let pk = prev[k];            
            if (!vk && !pk) continue;            
            if (!vk || !pk || parseInt(vk) != parseInt(pk)){
              ok = true;
              break;
            }               
          }//end of for
          if (ok) out[key] = val;          
        } else if (val != prev || key==PRIMARY_FIELD || key=="Họ và tên" || key=="Sinh ngày" || key=="Giới tính"){
          out[key] = val;
        }        
      }
      payload = JSON.stringify(out);      
    } else payload = JSON.stringify(values);
    // console.log("Sending ...");
    // console.log(payload);
